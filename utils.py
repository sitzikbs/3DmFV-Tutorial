    ...
    if type == 'grid':
        ...
        if not(isinstance(n_gaussians, list)):
            if D == 2:
                ...
                gmm = get_grid_gmm(subdivisions=[n_gaussians, n_gaussians, n_gaussians], variance=variance)
            else:
...
def get_grid_gmm(subdivisions=[5,5,5], variance=0.04):
    """
...
def fisher_vector(xx, gmm, normalization=True):
    ...
    s0 = np.sum(Q, 0)[:, np.newaxis]
    s1 = np.dot(Q.T, xx)
    s2 = np.dot(Q.T, xx ** 2)

    d_pi = (s0.squeeze() - n_points * gmm.weights_) / n_points * np.sqrt(gmm.weights_)
    d_mu = (s1 - gmm.means_ * s0 ) / n_points * np.sqrt(tiled_weights*gmm.covariances_)
    d_sigma = (
        + s2
        - 2 * s1 * gmm.means_
        + s0 * gmm.means_ ** 2
        - s0 * gmm.covariances_
        ) / (n_points * np.sqrt(2*tiled_weights)*gmm.covariances_)
...
def fisher_vector_per_point( xx, gmm):
    ...
    if D == 3:
        ...
        sig2 = np.array([gmm.covariances_.T[0, :], gmm.covariances_.T[1, :]]).T
    ...
    d_sigma =   np.tile(np.expand_dims(Q, -1), D)*((np.power(x_mu,2)/sig2_tiled)-1)/(np.sqrt(2)*sqrt_w)

    fv_per_point = np.squeeze(np.concatenate([np.expand_dims(d_pi, axis =-1), d_mu, d_sigma], axis=-1))
    return fv_per_point
...
def scale_to_unit_sphere(cloud):
    ...
    cloud = cloud - center
    scale = np.max(np.sqrt(np.sum(cloud**2, axis=1)))
    cloud = cloud / scale
    return cloud


def load_point_cloud_from_txt(pc_file_name):
    with open(pc_file_name) as f:
        pc = np.array([[float(element) for element in line.split()] for line in f])
    pc = scale_to_unit_sphere(pc[:, 0:3])
